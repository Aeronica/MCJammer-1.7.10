...
midi	: MIDI INT  			// match MIDI note
        | MIDI {notifyErrorListeners("Missing INT");}
        ;
...
cmd		: CMD INT
        | CMD {notifyErrorListeners("Missing INT");}
		;

len     : LEN INT
        | LEN INT DOT+
        | LEN {notifyErrorListeners("Missing INT");}
		;
...
// Lexer rules		
CMD		: [iotvIOTV] ;          // MML commands Instrument, Length, Octave, Tempo, Volume
