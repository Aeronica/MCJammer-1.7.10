...
import net.aeronica.mods.mcjammer.client.mml.util.MMLParser.AnoteContext;
import net.aeronica.mods.mcjammer.client.mml.util.MMLParser.BandContext;
import net.aeronica.mods.mcjammer.client.mml.util.MMLParser.InstContext;
import net.aeronica.mods.mcjammer.client.mml.util.MMLParser.LenContext;
/**
 * Transforms MML into a set of data structures that
 * can be used to generate MIDI, or other formats.
 * 
 * @author Paul Boese
 *
 */
public abstract class MMLTransformBase extends MMLBaseListener {
     ...
     * 
     * <code>
     * void processMObjects(List &lt;IMObjects&gt; mObjects) {<br>
     * <blockquote>IMObjects.Type type;<br><blockquote>
     * for(int i=0; i < mmlObjects.size(); i++) {<br><blockquote>
     *     System.out.println(getMObject(i));<br>
     *     switch (getMMLObject(i).getType()) {<br>
     *          case INST_BEGIN: {<br><blockquote>
     *              MmlInstBegin mmo = (MmlInstBegin) getMObject(i);<br>
     *              // Nothing to do in reality<br>
     *              break;<br>
     *          }<br></blockquote>
     *          case TEMPO: {<br><blockquote>
     *              MmlTempo mmo = (MmlTempo) getMObject(i);<br>
    ...
    @Override
    public void enterTied(MMLParser.TiedContext ctx) {
    ...
    public void enterCmd(MMLParser.CmdContext ctx) {
        ...
        int value = Integer.valueOf(ctx.INT().getText());
            switch (c) {
            case 'I': { instState.setInstrument(value);
                MOInst mmo = new MOInst(instState.getInstrument(), partState.getRunningTicks());
                saveMObject(mmo);                
            } break;
            case 'O': partState.setOctave(value); break;
            case 'T': { instState.setTempo(value);
                MOTempo mmo = new MOTempo(instState.getTempo(), partState.getRunningTicks());
                saveMObject(mmo);
            } break;
            case 'V': partState.setVolume(value); break;
        }
